...
namespace goldilock::test { 
  ...
  static auto TEST_DATA_perf_max_goldilock_queue = {
    1, // sequentially
    2,
    4,
    8,
  ...
  BOOST_DATA_TEST_CASE(goldilocked_speedtest, boost::unit_test::data::make(TEST_DATA_perf_max_goldilock_queue), td_queue_length) {
    auto wd = get_goldilock_case_working_dir();
    fs::create_directories(wd);

    const fs::path support_app_exiter = get_executable_path_from_test_env("support_app_exiter");
    ...
    std::chrono::steady_clock::time_point bench_end = bench_start + 10s;
    ...
    std::cout << "===============================" << std::endl;
    std::cout << "Creating as many tasks as possible for the next 10s (queue length limit: " << td_queue_length << ")" << std::endl;

    while(std::chrono::steady_clock::now() < bench_end) {

      // Pause the creation as we reach the queue length limit
      while(tasks_created - tasks_finished >= td_queue_length && std::chrono::steady_clock::now() < bench_end) {
        std::this_thread::sleep_for(1ms);
      }

      size_t task_ix = tasks_created++;

      
      std::chrono::steady_clock::time_point task_start = std::chrono::steady_clock::now();

      auto task = std::make_shared<task_wrapper>(task_ix, ios, 
        host_goldilock_executable_path(), "--lockfile", gl_A_lockfile, "--", support_app_exiter, "-r", "0",
        bp::start_dir=wd, bp::std_out > stdout, bp::std_err > stderr, bp::std_in < bp::null,
        bp::on_exit=[task_ix=task_ix, &tasks_finished, &tasks_failed, &task_start](int exit_code, std::error_code const& ec) {
          tasks_finished++;
    ...
    std::chrono::steady_clock::time_point bench_expiry = bench_end + 30s;

    std::cout << "Running for max 30 more seconds..." << std::endl;

    while(std::chrono::steady_clock::now() < bench_expiry && tasks_finished < tasks_created) {
      std::cout << "(status) tasks completed: " << tasks_finished << " out of " << tasks_created << std::endl;
      std::this_thread::sleep_for(1s);
    }

    BOOST_ASSERT(tasks_finished == tasks_created);

    std::chrono::steady_clock::time_point bench_finished = std::chrono::steady_clock::now();


    auto all_tasks_duration = std::chrono::duration_cast<std::chrono::milliseconds>(bench_finished - bench_start).count();
    std::chrono::duration<double> all_tasks_duration_seconds = bench_finished - bench_start;
    std::cout << "All tasks completed in: " << all_tasks_duration << "ms" << std::endl;
    std::cout << "Average time per tasks: " << (all_tasks_duration / tasks_finished)  << "ms" << std::endl;
    std::cout << "Tasks per second: " << (tasks_finished / (all_tasks_duration_seconds.count()))  << std::endl;
  ...
  BOOST_AUTO_TEST_CASE(goldilocked_write_many_instances_single_lock) {
    ...
    while(task_ix < tasks_expected) {
      ...
      auto task = std::make_shared<task_wrapper>(task_ix, ios, 
        host_goldilock_executable_path(), "--lockfile", gl_A_lockfile, "--", support_app_append_to_file_bin, "-s", std::to_string(task_ix) + ":"s, "-n", "5", "-f", write_output_dest.generic_string(), "-i", "1" ,
        bp::start_dir=wd, bp::std_out > stdout, bp::std_err > stderr, bp::std_in < bp::null,
  ...
  BOOST_AUTO_TEST_CASE(goldilocked_write_many_instances_multiple_locks) {
